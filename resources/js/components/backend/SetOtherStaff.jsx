...
export default function SetOtherStaff() {
    ...
    const currentpost = fetchedOtherStaff.slice(
        indexOfFirstPost,
        indexOfLastPost
    );
    ...
    // delete a post
    const removeTour = (id) => {
        const newImg = fetchedOtherStaff.filter((item) => item.id !== id);

        setfetchedOtherStaff(newImg);
    };
    // handling the image
    ...
    const handleEdit = (e, staffId) => {
        ...
        axios
            .post(`/api/OtherStaff/update/${staffId}`, fd, {
                headers: {
